...
def export_lb_assembly():
    print("=========================================================================")
    print(" EXPORTING NEW GAZEBO MODEL FROM LB.BLEND ONLY")
    print(" Master Source: /home/dk05/Desktop/SMART_HORIZON/LUNA_PRO/LB.blend")
    print("=========================================================================")
    ...
    excluded_names = ["preview_ground", "ground", "plane", "floor", "referenceplane", "shadowplane"]
    ...
    for gname, gobjs in groups.items():
        ...
        for o in gobjs:
            for corner in o.bound_box:
                w_corner = o.matrix_world @ mathutils.Vector(corner)
                for i in range(3):
                    min_v[i] = min(min_v[i], w_corner[i])
                    max_v[i] = max(max_v[i], w_corner[i])
                
        center = (min_v + max_v) / 2.0
        world_centers[gname] = center
        print(f"Group: {gname:20s} ({len(gobjs):3d} objs) | World Center: ({center.x:6.3f}, {center.y:6.3f}, {center.z:6.3f})")
    ...
    for gname, gobjs in groups.items():
        ...
        print(f"✅ EXPORTED ZERO-CENTERED GLB -> {export_path}")

    print("\n=========================================================================")
    print(" ALL 12 ZERO-CENTERED GLBS EXPORTED FROM LB.BLEND SUCCESSFULLY!")
    print("=========================================================================")
